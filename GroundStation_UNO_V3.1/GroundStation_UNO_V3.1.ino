...
int initial = false;
int eeAddress = EE_ADDRESS_BEGIN;                                          //address in EEPROM of calibration data
long bnoID;                                                 //Address of thisSensor
adafruit_bno055_offsets_t calibrationData, newCalib;
sensor_t thisSensor;
...
void setup() {
  ...
  if(!bno.begin())                                          //Launches the IMU. It returns a true value if it successfully launches. 
  {
    ...
    Serial.print("Ooops, no sensor connected!");
  }

  EEPROM.get(eeAddress, bnoID);

  bno.getSensor(&thisSensor);
    if (bnoID == thisSensor.sensor_id)//calibration data exists, load it in and resume testing.
    {
 ...
 if(calibrated == false){
  ...
  Serial.println(mag, DEC);
    if(bno.isFullyCalibrated()){
      ...
      //sensor has been calibrated. Write new values into EEPROM
      bno.getSensorOffsets(newCalib);
      eeAddress = EE_ADDRESS_BEGIN;
      bno.getSensor(&thisSensor);
      bnoID = thisSensor.sensor_id; //getting sensor ID
      EEPROM.put(eeAddress, bnoID); //setting EEPROM
      eeAddress +=sizeof(long);
      EEPROM.put(eeAddress, newCalib); //storing new calibratoion data in EEPROM
